  ...
  fluidRow(column=12,
           ...
           sidebarLayout(
             sidebarPanel(
               ...
               #              choices = c("New cases (7-day average)" = "new_cases_7dayavg",
               #                          "New cases per million (7-day average)" = "permill_7dayavg")),
               #country
...
server <- function(input, output) {
  output$mapPlot <- renderPlot({
    ...
    map1 <- ggplot(mapdat4, aes(x = long, y = lat, group = group)) +
      ...
      scale_fill_gradientn(n.breaks=8, colours=rev(heat.colors(10)),na.value="grey90") +
